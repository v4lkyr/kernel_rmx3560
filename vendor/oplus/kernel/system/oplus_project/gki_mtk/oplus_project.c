...
bool oplus_daily_build(void)
{
	...
	pr_err("build_variant is %s\n", build_variant);
	if (strstr(build_variant, "userdebug") || strstr(build_variant, "eng")) {
        daily_build = true;
    } else {
        daily_build = false;
    }
...
static int __init oplus_project_init(void)
{
	...
	pr_err("Roland oplus_project_init start\n");
