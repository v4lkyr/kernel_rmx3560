...
int oplus_gpio_suspend(struct touchpanel_data *ts)
{
	int r = 0;

	if (ts->hw_res.pinctrl) {
		r = pinctrl_select_state(ts->hw_res.pinctrl, ts->hw_res.pin_spi_mode_suspend);
		if (r < 0) {
			TPD_INFO("Failed to select int spi mode state, r:%d", r);
			goto err_suspend_pinctrl;
		}
		r = pinctrl_select_state(ts->hw_res.pinctrl, ts->hw_res.pin_int_sta_suspend);
		if (r < 0) {
			TPD_INFO("Failed to select int suspend state, r:%d", r);
			goto err_suspend_pinctrl;
		}
		r = pinctrl_select_state(ts->hw_res.pinctrl, ts->hw_res.pin_rst_sta_suspend);
		if (r < 0) {
			TPD_INFO("Failed to select rst suspend state, r:%d", r);
			goto err_suspend_pinctrl;
		}
		TPD_INFO("success get oplus gpio suspend pinctrl active state");
	}
err_suspend_pinctrl:
	return r;
}

int oplus_gpio_resume(struct touchpanel_data *ts)
{
	int r = 0;
	...
	if (ts->hw_res.pinctrl) {
		...
		if (r < 0) {
			TPD_INFO("Failed to select int active state, r:%d", r);
			goto err_resume_pinctrl;
		}
		r = pinctrl_select_state(ts->hw_res.pinctrl, ts->hw_res.pin_rst_sta_active);
		if (r < 0) {
			TPD_INFO("Failed to select rst active state, r:%d", r);
			goto err_resume_pinctrl;
		}
		TPD_INFO("success get oplus gpio resume pinctrl active state");
	}
err_resume_pinctrl:
		return r;
}
