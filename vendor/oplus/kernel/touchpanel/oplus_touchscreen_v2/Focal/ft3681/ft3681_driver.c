...
static int fts_set_high_frame_rate(void *chip_data, int level, int time)
{
	...
	ret = fts_enable_game_mode(ts_data, level);
...
static void fts_enable_gesture_mask(void *chip_data, uint32_t enable)
{
	...
	ret = ft3681_fts_write_reg(FTS_REG_GESTURE_CONFIG1, config1);

	if (ret < 0) {
		TPD_INFO("%s: write FTS_REG_GESTURE_CONFIG1 enable(%x=%x) fail", __func__,
			 FTS_REG_GESTURE_CONFIG1, config1);
	}

	ret = ft3681_fts_write_reg(FTS_REG_GESTURE_CONFIG2, config2);

	if (ret < 0) {
		TPD_INFO("%s: write FTS_REG_GESTURE_CONFIG2 enable(%x=%x) fail", __func__,
			 FTS_REG_GESTURE_CONFIG2, config2);
	}

	ret = ft3681_fts_write_reg(FTS_REG_GESTURE_CONFIG4, config4);

	if (ret < 0) {
		TPD_INFO("%s: write FTS_REG_GESTURE_CONFIG4 enable(%x=%x) fail", __func__,
			 FTS_REG_GESTURE_CONFIG4, config4);
	}

	msleep(1);
	TPD_INFO("%s, enable[%d] register[FTS_REG_GESTURE_CONFIG1. FTS_REG_GESTURE_CONFIG2. FTS_REG_GESTURE_CONFIG4]",
		 __func__, enable);
}
...
static int fts_get_rawdata_tmp(struct chip_data_ft3681 *ts_data, int *raw,
			   bool is_diff, u8 *buf)
{
	int ret = 0;
	int i = 0;
...
static void fts_delta_snr_read(struct seq_file *s, void *chip_data, uint32_t count)
{
	...
	TPD_INFO("%s, disable_irq %d \n", __func__, ts_data->ts->irq);
	disable_irq(ts_data->ts->irq);
	mdelay(10);

	ret = ft3681_fts_write_reg(0x9E, 0x81);
	...
	TPD_INFO("%s:open fastdiff", __func__);


	for (i = 0; i < count; i++) {
		ret = fts_get_rawdata_tmp(ts_data, raw, true, buf);
		if (ret < 0) {
			seq_printf(s, "get diff data fail\n");
			continue;
		}
		...
		for (j = 0; j < 10; j++) {
			if (snr[j].point_status) {
				if (i) {
					snr[j].max = raw[rx_num * snr[j].channel_x + snr[j].channel_y] > snr[j].max ?
						raw[rx_num * snr[j].channel_x + snr[j].channel_y] : snr[j].max;
					snr[j].min = raw[rx_num * snr[j].channel_x + snr[j].channel_y] < snr[j].min ?
						raw[rx_num * snr[j].channel_x + snr[j].channel_y] : snr[j].min;
				} else {
				...
				TPD_INFO("snr%d report %d += %d \n", j, snr[j].sum,
					raw[rx_num * snr[j].channel_x + snr[j].channel_y]);
			}
		}
	}
	...
	ret = ft3681_fts_write_reg(0x9E, 0x00);
	...
	TPD_INFO("%s:close fastdiff", __func__);

	mdelay(10);
	enable_irq(ts_data->ts->irq);
	TPD_INFO("%s, enable_irq %d \n", __func__, ts_data->ts->irq);

set_fail:
	ft3681_fts_write_reg(0x00, 0x00);
