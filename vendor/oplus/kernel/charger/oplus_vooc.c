...
*/
	...
	if (chip->hiz_gnd_cable) {
		...
		if (oplus_chg_get_bcc_support() && g_vooc_chip->bcc_wake_up_done == false) {
...
static int init_vooc_proc(struct oplus_vooc_chip *chip)
{
	...
	} else if (get_project()== 136895) {
		snprintf(chip->fw_path, MAX_FW_NAME_LENGTH, "fastchg/216BF/oplus_vooc_fw_op10.bin");
	} else if (get_project()== 136894) {
		snprintf(chip->fw_path, MAX_FW_NAME_LENGTH, "fastchg/216BE/oplus_vooc_fw_op10.bin");
	} else if (get_project()== 136902) {
		snprintf(chip->fw_path, MAX_FW_NAME_LENGTH, "fastchg/216C6/oplus_vooc_fw_op10.bin");
	} else if (get_project()== 136903) {
		snprintf(chip->fw_path, MAX_FW_NAME_LENGTH, "fastchg/216C7/oplus_vooc_fw_op10.bin");
	} else if (get_project()== 136933) {
		snprintf(chip->fw_path, MAX_FW_NAME_LENGTH, "fastchg/216E5/oplus_vooc_fw_op10.bin");
	} else if (get_project()== 136934) {
		snprintf(chip->fw_path, MAX_FW_NAME_LENGTH, "fastchg/216E6/oplus_vooc_fw_op10.bin");
	} else {
