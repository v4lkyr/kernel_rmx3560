	...
	int last_usb_temp_l, int last_usb_temp_r, int batt_current)
{
	...
	if (condition == USBTEMP_TRIGGER_CONDITION_1) {
		...
		index += snprintf(&(chip->usbtemp_load_trigger.crux_info[index]),
				...
				g_tbatt_temp, chip->usb_temp_l, last_usb_temp_l,
...
void oplus_update_usbtemp_current_status(struct oplus_chg_chip *chip)
{
	...
	if ((chip->usb_temp_r  - g_tbatt_temp/10) >= chip->usbtemp_cool_down_temp_gap) {
		limit_cur_cnt_r++;
		recover_cur_cnt_r = 0;
	} else if ((chip->usb_temp_r  - g_tbatt_temp/10) <= chip->usbtemp_cool_down_recovery_temp_gap)  {
		recover_cur_cnt_r++;
		limit_cur_cnt_r = 0;
	}

	if ((chip->usb_temp_l  - g_tbatt_temp/10) >= chip->usbtemp_cool_down_temp_gap) {
		limit_cur_cnt_l++;
		recover_cur_cnt_l = 0;
	} else if ((chip->usb_temp_l  - g_tbatt_temp/10) <= chip->usbtemp_cool_down_recovery_temp_gap)  {
		recover_cur_cnt_l++;
		limit_cur_cnt_l = 0;
	}
...
int oplus_usbtemp_monitor_common(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		if (g_tbatt_temp/10 > USB_50C && (((chip->usb_temp_l >= g_tbatt_temp/10 + 7) && (chip->usb_temp_l < USB_100C))
			|| ((chip->usb_temp_r >= g_tbatt_temp/10 + 7) && (chip->usb_temp_r < USB_100C)))) {
			pr_err("in loop 1");
			for (i = 1; i <= retry_cnt; i++) {
				mdelay(RETRY_CNT_DELAY);
				get_usb_temp(chip);
				if ((chip->usb_temp_r >= g_tbatt_temp/10 + 7) && chip->usb_temp_r < USB_100C)
					count_r++;
				if ((chip->usb_temp_l >= g_tbatt_temp/10 + 7) && chip->usb_temp_l < USB_100C)
					count_l++;
				pr_err("countl : %d", count_l);
			}
			if (count_r >= retry_cnt || count_l >= retry_cnt) {
...
bool oplus_usbtemp_l_trigger_current_status(struct oplus_chg_chip *chip)
{
	...
	if (chip->usbtemp_curr_status == OPLUS_USBTEMP_LOW_CURR) {
		if ((chip->usb_temp_l >= chip->usbtemp_cool_down_ntc_low) ||
			(chip->usb_temp_l - g_tbatt_temp / 10) >=
...
bool oplus_usbtemp_r_trigger_current_status(struct oplus_chg_chip *chip)
{
	...
	if (chip->usbtemp_curr_status == OPLUS_USBTEMP_LOW_CURR) {
		if ((chip->usb_temp_r >= chip->usbtemp_cool_down_ntc_low) ||
			(chip->usb_temp_r - g_tbatt_temp / 10) >=
				chip->usbtemp_cool_down_gap_low)
			return true;
		return false;
	} else if (chip->usbtemp_curr_status == OPLUS_USBTEMP_HIGH_CURR) {
...
bool oplus_usbtemp_condition_temp_high(struct oplus_chg_chip *chip)
{
	...
	if (chip->usbtemp_curr_status == OPLUS_USBTEMP_LOW_CURR) {
		if (g_tbatt_temp / 10 <= chip->usbtemp_batt_temp_low &&
		...
		if (g_tbatt_temp / 10 <= chip->usbtemp_batt_temp_high &&
...
bool oplus_usbtemp_temp_rise_fast_without_batt_temp(struct oplus_chg_chip *chip)
{
	...
	if (chip->usbtemp_curr_status == OPLUS_USBTEMP_LOW_CURR) {
		if ((((chip->usb_temp_l - g_tbatt_temp / 10) >
					...
					int usbtemp_first_time_in_curr_range, bool curr_range_change)
{
	...
	if (((current_curr_change_temp_l - last_curr_change_usb_temp_l) >= OPLUS_USBTEMP_CURR_CHANGE_TEMP)
		...
		if ((count_curr_l >= retry_cnt &&  chip->usb_temp_l > USB_30C && chip->usb_temp_l < USB_100C)
			...
			chg_err("change curr range...,current_temp_l=%d,last_usb_temp_l=%d,current_temp_r=%d,last_usb_temp_r =%d, g_tbatt_temp = %d\n",
