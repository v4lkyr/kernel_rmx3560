...
extern int nu1619_driver_init(void);
...
void oplus_chg_choose_gauge_curve(int index_curve)
{
	...
	if (target_index_curve != last_curve_index) {
		...
		//set_charge_power_sel(target_index_curve);//bringup 6895
...
bool oppo_pps_check_authen_data(void)
{
	uint32_t  key[4] = {0};
...
{	
	...
	if (ret == TCPM_SUCCESS) {
		...
		if ((imax > OPLUS_EXTEND_IMIN) && (vmax >= OPLUS_EXTEND_VMIN )) {
			...
			return true;
		} else if ((imax > OPLUS_PPS_IMIN) && (vmax >= OPLUS_EXTEND_VMIN )) {
...
void oplus_pps_get_adapter_status(struct oplus_chg_chip *chip)
{
	...
	chip->pd_authentication = oppo_pps_check_authen_data();
	...
	oplus_pps_tcpc_set_dr(chip, TYPEC_DEVICE);
	chg_err("oplus_pps_get_adapter_status 1\n");
...
#endif
	chg_err("charg_subtype = %d, pd_type = %d\n", charg_subtype, pinfo->pd_type);
...
static struct device_attribute *oplus_mtk6895_cust_battery_attributes[] = {
	&dev_attr_soc_ajust,
	NULL
};
...
static int mtk_charger_probe(struct platform_device *pdev)
{
	...
	if (oplus_chip->nightstandby_support) {
		oplus_battery_node_add(oplus_mtk6895_cust_battery_attributes);
	}
...
static int mtk_charger_remove(struct platform_device *dev)
{
	...
	if (g_oplus_chip->nightstandby_support) {
		oplus_battery_node_delete(oplus_mtk6895_cust_battery_attributes);
	}
	return 0;
}
