...
static int bq27541_get_batt_soc_real(void)	/*  yzj20220109  */
{
	if (!gauge_ic) {
		return 0;
	}
	return gauge_ic->soc_real;
}
...
*/
	...
	/*this is add for pps projects gague convert */
	gauge_ic->soc_real = soc;
	soc = ((soc * 10000 / chip->chip_soc_convert_support) + 5) / 10;
	/*this is add for pps projects gague convert */
...
static int bq28z610_get_battery_bcc_parameters(char *buf)
{
	...
	if ((oplus_vooc_get_fastchg_ing() && oplus_vooc_get_fast_chg_type() != CHARGER_SUBTYPE_FASTCHG_VOOC) || oplus_pps_get_pps_fastchg_started()) {
		buffer[15] = 1;
	} else {
		buffer[15] = 0;
	}
	...
	if ((oplus_vooc_get_fastchg_ing() && oplus_vooc_bcc_get_temp_range() == false)
...
static struct oplus_gauge_operations bq27541_gauge_ops = {
	...
	.get_battery_soc_real = bq27541_get_batt_soc_real,
...
static void bq27541_parse_dt(struct chip_bq27541 *chip)
{
	...
	rc = of_property_read_u32(node, "qcom,chip_soc_convert_support", &chip->chip_soc_convert_support);
	if(rc) {
		chip->chip_soc_convert_support = 1000;
	}
}
