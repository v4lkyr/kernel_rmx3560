...
static void oplus_chg_update_ui_soc(struct oplus_chg_chip *chip)
{
	...
	if (chip->nightstandby_support) {
		...
		if ((!get_soc_feature()) && (soc_callback_in == true)) {
			...
			chg_err("soc_callback_in=%d \r\n", soc_callback_in);
		}

		if ((!get_soc_feature()) && (soc_callback_mark == true)) {
			g_charger_chip->fourth_ktime = ktime_to_ms(ktime_get_boottime()) / 1000;
			chg_err("interval: [%d], thrid_ktime:[%d], fourth_ktime:[%d]\n", (g_charger_chip->fourth_ktime - g_charger_chip->thrid_ktime),
				g_charger_chip->thrid_ktime, g_charger_chip->fourth_ktime);
			if ((chip->soc > SOC_AJUST_MINIMUM_BATTERY) && (chip->ui_soc > chip->soc)
			    && (SOC_AJUST_BAT_TEMP_ABNORMAL_COOL <= chip->temperature) && (chip->temperature <= SOC_AJUST_BAT_TEMP_ABNOMAL_WARM)) {
				chg_err("diminishing real returns[%d]\n", (chip->ui_soc - chip->soc));
				if (((g_charger_chip->fourth_ktime - g_charger_chip->thrid_ktime) >= SOC_AJUST_DELAY_8MINTUTE)
					&& (t_count_8min == true)) {
					t_count_8min = false;
					chip->ui_soc--;
					g_charger_chip->thrid_ktime = g_charger_chip->fourth_ktime;
					chg_err("delay 8 minute chip->ui_soc=%d \r\n", chip->ui_soc);
				}

				if (((g_charger_chip->fourth_ktime - g_charger_chip->thrid_ktime) >= SOC_AJUST_DELAY_INTERVAL_2MINTUTE)
					&& (t_count_8min == false)) {
					chip->ui_soc--;
					g_charger_chip->thrid_ktime = g_charger_chip->fourth_ktime;
					chg_err("based on 8 minutes, interval 2 minute chip->ui_soc=%d \r\n", chip->ui_soc);
				}
			} else {
				chg_err("soc_callback_mark=%d \r\n", soc_callback_mark);
				...
				if (soc_reduce_margin == 0) {
					...
					if (chip->save_sleep_tm_sec >= ONE_MINUTE
						...
						if (!chip->nightstandby_support) {
							...
							if ((soc_callback_mark == false) || ((SOC_AJUST_NIGHTCOUNT_LOW <= night_count) && (night_count <= SOC_AJUST_NIGHTCOUNT_HIGH))) {
								chip->ui_soc--;
							}
						}
						soc_down_count = 0;
...
int oplus_chg_match_temp_for_chging(void)
{
	...
	if (oplus_switching_support_parallel_chg()) {
		...
		if ((oplus_pps_get_support_type() == PPS_SUPPORT_2CP || g_charger_chip->vooc_project == DUAL_BATT_100W)
			...
			if(oplus_gauge_get_batt_temperature() == -400)
				batt_temp = -400;
			if (get_eng_version() == HIGH_TEMP_AGING || oplus_is_ptcrb_version()) {
