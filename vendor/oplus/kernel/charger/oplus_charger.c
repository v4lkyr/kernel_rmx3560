...
#define OPLUS_CHG_UPDATE_INTERVAL_SEC_0 		1
...
#define OPLUS_CHG_UPDATE_INTERVAL_0	round_jiffies_relative(msecs_to_jiffies(OPLUS_CHG_UPDATE_INTERVAL_SEC_0*1000))
...
u8 soc_store[4]= {0};
u8 night_count = 0;
bool soc_callback_mark = false;
bool soc_callback_in = false;
...
static void oplus_chg_set_charging_current(struct oplus_chg_chip *chip)
{
	...
	switch (chip->tbatt_status) {
		...
		case BATTERY_STATUS__COLD_TEMP:
			...
			if (vbatt_higherthan_4180mv) {
				if (chip->temperature >= chip->limits.freeze_bat_decidegc)
	...
	charger_xlog_printk(CHG_LOG_CRTI, "oplus_chg_set_charging_current: %d\n", charging_current);
...
static void oplus_chg_set_float_voltage(struct oplus_chg_chip *chip)
{
	...
	charger_xlog_printk(CHG_LOG_CRTI, "oplus_chg_set_float_voltage, set float voltage = %d\n", flv);
	if (oplus_voocphy_get_bidirect_cp_support() == false) {
...
void oplus_chg_turn_on_charging(struct oplus_chg_chip *chip)
{
	...
	chg_err("oplus_chg_turn_on_charging--------\r\n");
...
static void oplus_chg_check_tled_status(struct oplus_chg_chip *chip)
{
	...
	if (tled_status != chip->led_temp_status) {
		...
		chg_err("tled status change, [%d %d %d %d %d]\n",
				tled_status,
				chip->led_temp_status,
				chip->limits.led_warm_bat_decidegc_antishake,
				chip->limits.led_high_bat_decidegc_antishake,
...
void oplus_charger_detect_check(struct oplus_chg_chip *chip)
{
	...
	if (chip->chg_ops->check_chrdet_status()) {
...
#endif
			chg_err("Charger in,POWER_SUPPLY_TYPE_UNKNOWN charger_type=%d\n", chip->charger_type);
		} else {
		...
		chg_err("Charger in,check_chrdet_status =%d\n", chip->chg_ops->check_chrdet_status());
...
bool oplus_check_afi_update_condition(void)
{
	...
	if (chip->charger_exist) {
		...
		if (CHARGER_SUBTYPE_FASTCHG_VOOC == chg_fast_type) {	/*vooc*/
			chg_err(" true 1: vooc\n");
			return true;
		} else {
			if (chg_fast_type) {								/*svooc*/
...
static void oplus_chg_get_battery_data(struct oplus_chg_chip *chip)
{
	...
	if (oplus_chg_get_voocphy_support() == NO_VOOCPHY && oplus_vooc_get_fastchg_started() == true) {
		...
		chip->batt_rm = oplus_gauge_get_prev_remaining_capacity() * chip->vbatt_num;
		chip->soc_real = oplus_gauge_get_batt_soc_real();
	} else {
		if (oplus_plat_gauge_is_support() && chip->mmi_chg != 0
				&& (mcu_status == 1 && oplus_vooc_get_reset_gpio_status() == 1)) {
			if (chip->charger_exist) {
...
static void oplus_chg_update_ui_soc(struct oplus_chg_chip *chip)
{
	...
	if ((!get_soc_feature())&&(soc_callback_in == true)) {
		g_charger_chip->thrid_ktime = ktime_to_ms(ktime_get_boottime())/1000;
		soc_callback_in = false;
	}
	if ((!get_soc_feature())&&(soc_callback_mark == true)&&(chip->ui_soc > chip->soc)) {
		g_charger_chip->fourth_ktime = ktime_to_ms(ktime_get_boottime())/1000;
		if (((g_charger_chip->fourth_ktime - g_charger_chip->thrid_ktime) <= 740)&&(chip->ui_soc < 95)
			&&(200 <= chip->temperature)&&(chip->temperature <= 350)) {
			if (((chip->ui_soc > chip->soc)&&((g_charger_chip->fourth_ktime - g_charger_chip->thrid_ktime)>= 480)
				&&((g_charger_chip->fourth_ktime - g_charger_chip->thrid_ktime) <= 500))&&(t_count_480s == true)) {
				t_count_480s = false;
				chip->ui_soc--;
				printk("delay 8 minute chip->ui_soc=%d \r\n", chip->ui_soc);
			}
			if (((chip->ui_soc > chip->soc)&&((g_charger_chip->fourth_ktime - g_charger_chip->thrid_ktime) >= 600)
				&&((g_charger_chip->fourth_ktime - g_charger_chip->thrid_ktime) <= 620))&&(t_count_600s == true)) {
				t_count_600s = false;
				chip->ui_soc--;
				printk("delay 10 minute chip->ui_soc=%d \r\n", chip->ui_soc);
			}
			if (((chip->ui_soc > chip->soc)&&((g_charger_chip->fourth_ktime - g_charger_chip->thrid_ktime) >= 720)
				&&((g_charger_chip->fourth_ktime - g_charger_chip->thrid_ktime) <= 740))&&(t_count_720s == true)) {
				t_count_720s = false;
				chip->ui_soc--;
				printk("delay 12 minute chip->ui_soc=%d \r\n", chip->ui_soc);
			}
		} else {
			soc_callback_mark = false;
			g_charger_chip->thrid_ktime = 0;
			g_charger_chip->fourth_ktime = 0;
			if (chip->ui_soc == 100) {
	...
	if (chip->charger_exist && chip->batt_exist && chip->batt_full && chip->mmi_chg && (chip->stop_chg == 1 || chip->charger_type == 5)
		...
		if (chip->ui_soc != ui_soc_pre) {
			chg_debug("full [soc ui_soc smooth_soc up_limit] = [%d %d %d %d]\n",
				...
				&& chip->mmi_chg && (chip->stop_chg == 1 || chip->charger_type == 5) && !oplus_pps_get_pps_disconnect()) {
#else
			...
			if (chip->sleep_tm_sec > 0) {
				...
				if (soc_reduce_margin == 0) {
					if ((chip->ui_soc - chip->smooth_soc) > 2) {
						if ((soc_callback_mark == false) || ((1 <= night_count) && (night_count <= 4))) {
							chip->ui_soc--;
						}
						soc_down_count = 0;
						chip->sleep_tm_sec = 0;
...
/*10 seconds after typec init, PD starts to communicate, so set true force reset adapter.only for MTK*/
	...
	if (oplus_pps_get_chg_status() != PPS_NOT_SUPPORT && chip->pps_force_svooc == false
		...
		chg_err("PD_PPS_ACTIVE,switch return\n");/*debug for pd*/
		return;
	}
...
static bool oplus_chg_check_ffc_status(struct oplus_chg_chip *chip)
{
	...
	if (chip->fastchg_to_ffc == true) {
		...
		if (chip->fastchg_ffc_status == 1) {
			chg_err("vffc1_counts = %d, normal_counts = %d, warm_counts = %d, batt_volt = %d, ffc1_normal_vfloat_sw_limit = %d, \
				chip->temperature = %d, chip->ffc_temp_status = %d, chip->icharging = %d, chip->ff1_normal_fastchg_ma = %d, \
				chip->ff1_warm_fastchg_ma = %d\n",
				vffc1_counts, normal_counts, warm_counts, chip->batt_volt, chip->limits.ffc1_normal_vfloat_sw_limit, chip->temperature,
				chip->ffc_temp_status, chip->icharging, chip->limits.ff1_normal_fastchg_ma, chip->limits.ff1_warm_fastchg_ma);
		...
		if (chip->fastchg_ffc_status == 2) {
			chg_err("vffc2_counts = %d, normal_counts = %d, warm_counts = %d, batt_volt = %d, ffc2_normal_vfloat_sw_limit = %d, \
				ffc2_warm_vfloat_sw_limit = %d, chip->temperature = %d, chip->ffc_temp_status = %d, chip->icharging = %d, \
				chip->ffc2_normal_fastchg_ma = %d, chip->ffc2_warm_fastchg_ma = %d\n",
				vffc2_counts, normal_counts, warm_counts, chip->batt_volt, chip->limits.ffc2_normal_vfloat_sw_limit,
				chip->limits.ffc2_warm_vfloat_sw_limit, chip->temperature, chip->ffc_temp_status, chip->icharging,
				chip->limits.ffc2_normal_fastchg_ma, chip->limits.ffc2_warm_fastchg_ma);
			if (chip->ffc_temp_status == FFC_TEMP_STATUS__WARM)
				ffc_vfloat_sw_limit = chip->limits.ffc2_warm_vfloat_sw_limit;
			else
				ffc_vfloat_sw_limit = chip->limits.ffc2_normal_vfloat_sw_limit;
...
static void oplus_chg_print_log(struct oplus_chg_chip *chip)
{
	if(chip->vbatt_num == 1){
		charger_xlog_printk(CHG_LOG_CRTI,
			"CHGR[ %d / %d / %d / %d / %d ], "
			"BAT[ %d / %d / %d / %d / %d / %d ], "
			"GAUGE[ %d  %d / %d / %d / %d / %d / %d / %d / %d / %d / %d / %d / %d / %d / %d / %d / %d / %d / %d / %d / %d / %d / %d / %d /], "
			...
			chip->ibus, chip->soc, chip->ui_soc, chip->soc_load, chip->soc_real, chip->batt_rm,
...
{
	chg_err("oplus_chg_pd_config,chip->pd_svooc = %d,pd_chging = %d,pps_force_svooc = %d\n", chip->pd_svooc, chip->pd_chging, chip->pps_force_svooc);
...
static void oplus_chg_pd_config(struct oplus_chg_chip *chip)
{
	...
	if (oplus_pps_get_ffc_started() == true) {
		chg_err("oplus_chg_pd_config---oplus_pps_get_ffc_started == true\n");
		return;
	}
...
int oplus_convert_pps_current_to_level(struct oplus_chg_chip *chip, int val)
{
	...
	level = find_level_to_current(val, pps_oppo, ARRAY_SIZE(pps_oppo));
	chg_err("level = %d\n", level);
	return level;
}
...
int oplus_chg_match_temp_for_chging(void)
{
	...
	if (oplus_switching_support_parallel_chg()) {
		...
		if (oplus_pps_get_support_type() == PPS_SUPPORT_2CP) {
			batt_temp = oplus_force_get_subboard_temp();
		...
		chg_err("shell_temp = %d, chging_temp = %d, batt_temp = %d\n", shell_temp, chging_temp, batt_temp);
		if (get_eng_version() == HIGH_TEMP_AGING) {
...
static void soc_modfiy_callback(void)
{
	...
	soc_store[2] = 1;
	if (get_soc_feature()) {
		soc_callback_in = true;
		soc_callback_mark = true;
		t_count_480s = true;
		t_count_600s = true;
		t_count_720s = true;
		soc_ajusting = true;
...
bool oplus_chg_get_bcc_support(void)
{
	if (!g_charger_chip) {
		...
		return g_charger_chip->smart_chg_bcc_support;
	}
}
