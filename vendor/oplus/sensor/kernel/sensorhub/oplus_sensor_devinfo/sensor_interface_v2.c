...
static int send_lcdinfo_to_hub(struct als_info *lcd_info)
{
    ...
    DEVINFO_LOG("type %u brightness %u, dc_mode %u", lcd_info->senstype,
        (uint32_t)lcd_info->brightness, (uint32_t)lcd_info->dc_mode);
    data = (uint32_t)lcd_info->brightness << 16 | lcd_info->dc_mode;

    return oplus_send_comm_to_hub(lcd_info->senstype, OPLUS_ACTION_SET_LCD_INFO, &data, sizeof(data));
}
