...
static void get_lcdinfo_brocast_type(void)
{
	if (si && si->is_sensor_type_available && si->is_sensor_type_available(SENSOR_TYPE_RGBW)) {
		g_als_info.senstype = SENSOR_TYPE_RGBW;
	} else if (si && si->is_sensor_type_available && si->is_sensor_type_available(SENSOR_TYPE_FRONT_CCT)) {
		g_als_info.senstype = SENSOR_TYPE_FRONT_CCT;
	} else {
		g_als_info.senstype = SENSOR_TYPE_RGBW;
	}
	DEVINFO_LOG("g_als_info.senstype %u", g_als_info.senstype);
}
